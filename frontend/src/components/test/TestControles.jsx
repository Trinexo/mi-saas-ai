      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: '1.5rem' }}>
        ...
        {feedbackMode && !confirmed && (
          <button
            disabled={!hasAnswer}
            onClick={onComprobar}
            style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: '#6366f1', color: '#fff', fontWeight: 600, cursor: hasAnswer ? 'pointer' : 'not-allowed', opacity: hasAnswer ? 1 : 0.5 }}
          >
            Comprobar
          </button>
        )}
