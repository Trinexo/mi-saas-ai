export default function TestControles({ index, total, onPrev, onNext, onSubmit, submitting, answered, error, feedbackMode = false, confirmed = false, hasAnswer = false }) {
  return (
    <>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: '1.5rem' }}>
        ...
        {(!feedbackMode || index < total - 1) && (
          <button
            onClick={onNext}
            disabled={!feedbackMode && index === total - 1}
            style={{ padding: '8px 18px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#f9fafb', color: '#374151', cursor: 'pointer', opacity: (!feedbackMode && index === total - 1) ? 0.5 : 1 }}
          >
            Siguiente
          </button>
        )}
