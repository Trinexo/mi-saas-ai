...
export default function ProfesorSimulacrosPage() {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try { await profesorApi.deleteMiSimulacro(token, deleteTarget.id); setDeleteTarget(null); load(); }
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <button onClick={openCreate}
          style={{ background: '#ea580c', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' }}>
          + Nuevo simulacro
        </button>
      </div>
      ...
      <div style={SECTION}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '3rem', color: '#9ca3af' }}>
            ...
            <button onClick={openCreate} style={{ marginTop: 16, background: '#ea580c', color: '#fff', border: 'none', borderRadius: 9, padding: '9px 20px', fontWeight: 700, cursor: 'pointer' }}>
              Crear el primero
            </button>
          </div>
        ) : (
                ...
                {items.map(s => (
                  <tr key={s.id}
                    ...
                    <td style={TD}>
                      ...
                      {s.oposicion_nombre && <div style={{ fontSize: '0.75rem', color: '#9ca3af', marginTop: 2 }}>{s.oposicion_nombre}</div>}
                    </td>
                    <td style={TD}>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 520, boxShadow: '0 8px 32px rgba(0,0,0,.18)', maxHeight: '90vh', overflowY: 'auto' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div>
                ...
                <input type="number" value={form.oposicion_id} onChange={e => setForm(p => ({ ...p, oposicion_id: e.target.value }))}
                  style={inputStyle} placeholder="(solo asignadas)" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" value={form.puntuacion_maxima} onChange={e => setForm(p => ({ ...p, puntuacion_maxima: e.target.value }))} style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Penalización (0-1)</label>
                <input type="number" step="0.01" min="0" max="1" value={form.penalizacion} onChange={e => setForm(p => ({ ...p, penalizacion: e.target.value }))} style={inputStyle} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 20 }}>
                <input type="checkbox" id="mrf_p" checked={form.mostrar_resultados_al_final} onChange={e => setForm(p => ({ ...p, mostrar_resultados_al_final: e.target.checked }))} />
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
              ...
              <button onClick={handleSave} disabled={saving}
                style={{ padding: '9px 22px', borderRadius: 9, border: 'none', background: '#ea580c', color: '#fff', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
        ...
        <div style={MODAL_OVERLAY} onClick={() => setDeleteTarget(null)}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 380, boxShadow: '0 8px 32px rgba(0,0,0,.18)' }}
            ...
            <p style={{ margin: '0 0 20px', color: '#6b7280', fontSize: '0.9rem' }}>
              Se eliminará <strong>{deleteTarget.nombre}</strong> con todos sus bloques y preguntas asignadas.
            </p>
