...
const TIPO_ICON = {
  reporte_actualizado: '!',
  plan_estudio: 'P',
  plan_estudio_recordatorio: 'P',
  sistema: 'i',
};

const TIPO_COLOR = {
  reporte_actualizado: { bg: '#eff6ff', border: '#bfdbfe', color: '#1e40af' },
  plan_estudio: { bg: '#f5f3ff', border: '#ddd6fe', color: '#6d28d9' },
  plan_estudio_recordatorio: { bg: '#fff7ed', border: '#fed7aa', color: '#c2410c' },
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              background: soloNoLeidas ? '#eff6ff' : '#fff',
              color: soloNoLeidas ? '#1d4ed8' : '#374151',
              fontWeight: soloNoLeidas ? 600 : 400,
              fontSize: '0.85rem',
            }}
          >
            {soloNoLeidas ? 'Solo no leidas' : 'Todas'}
          </button>
          {noLeidasCount > 0 && (
            <button
              disabled={markingAll}
      ...
      {!loading && items.length === 0 && (
        <div style={{ background: '#fff', borderRadius: 10, padding: '3rem', textAlign: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: '0.875rem', marginTop: 6 }}>
            {soloNoLeidas ? 'No hay notificaciones sin leer.' : 'Aqui apareceran las actualizaciones sobre tus reportes.'}
          </p>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map((n) => {
            ...
            return (
              ...
              >
                <span style={{ fontSize: '1.1rem', flexShrink: 0, lineHeight: 1.3, color: estilo.color, fontWeight: 800 }}>{icono}</span>
